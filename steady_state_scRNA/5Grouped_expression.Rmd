...
```{r}
#BM
BM.list <- read_xlsx("gene_lists/Res-BM/Res-BM.xlsx", sheet = "ranked up in BM")
BM.genes <- BM.list$Symbol
BM_mac <- intersect(total.gene.list, BM.genes)

# Get summed expression of genes of interest per cell
agg.BM_mac <- colSums(x = mac.wt@assays$RNA@data[BM_mac, ], na.rm = TRUE)

# Add summed expression values in 'object@meta.data$ROS.score'
if (all(names(x = agg.BM_mac) == rownames(x = mac.wt@meta.data))) {
  mac.wt@meta.data$BM_mac <- agg.BM_mac
}

#Res
Res.list <- read_xlsx("gene_lists/Res-BM/Res-BM.xlsx", sheet = "ranked up in Res")
Res.genes <- Res.list$Symbol
Res_mac <- intersect(total.gene.list, Res.genes)

# Get summed expression of genes of interest per cell
agg.Res_mac <- colSums(x = mac.wt@assays$RNA@data[Res_mac, ], na.rm = TRUE)

# Add summed expression values in 'object@meta.data$ROS.score'
if (all(names(x = agg.Res_mac) == rownames(x = mac.wt@meta.data))) {
  mac.wt@meta.data$Res_mac <- agg.Res_mac
}
