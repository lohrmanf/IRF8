...
cl1.wt.markers <- cl1.wt.markers[cl1.wt.markers$p_val_adj < 0.05, , drop = FALSE]

cl2.wt.markers <- FindMarkers(mac.wt, ident.1 = "2", min.pct = 0.25, logfc.threshold = 0.25)
cl2.wt.markers <- cl2.wt.markers[cl2.wt.markers$p_val_adj < 0.05, , drop = FALSE]

cl3.wt.markers <- FindMarkers(mac.wt, ident.1 = "3", min.pct = 0.25, logfc.threshold = 0.25)
cl3.wt.markers <- cl3.wt.markers[cl3.wt.markers$p_val_adj < 0.05, , drop = FALSE]

cl4H.wt.markers <- FindMarkers(mac.wt, ident.1 = "4H", min.pct = 0.25, logfc.threshold = 0.25)
cl4H.wt.markers <- cl4H.wt.markers[cl4H.wt.markers$p_val_adj < 0.05, , drop = FALSE]

cl4I.wt.markers <- FindMarkers(mac.wt, ident.1 = "4I", min.pct = 0.25, logfc.threshold = 0.25)
cl4I.wt.markers <- cl4I.wt.markers[cl4I.wt.markers$p_val_adj < 0.05, , drop = FALSE]

cl4C.wt.markers <- FindMarkers(mac.wt, ident.1 = "4C", min.pct = 0.25, logfc.threshold = 0.25)
cl4C.wt.markers <- cl4C.wt.markers[cl4C.wt.markers$p_val_adj < 0.05, , drop = FALSE]

cl4R.wt.markers <- FindMarkers(mac.wt, ident.1 = "4R", min.pct = 0.25, logfc.threshold = 0.25)
cl4R.wt.markers <- cl4R.wt.markers[cl4R.wt.markers$p_val_adj < 0.05, , drop = FALSE]
```
...
# Filter for shortlisted GO terms
```{r}

selected_cols <- combined_results[combined_results$NES > 0, c("ID", "Description", "NES", "Cluster")]
...
missing_go <- setdiff(selected_go_terms, selected_cols$ID)
```
...
missing <- setdiff(selected_go_terms, filtered_results$ID)
if (length(missing) > 0) message("Missing GO terms: ", paste(missing, collapse = ", "))
